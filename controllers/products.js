...
exports.getProducts = asyncHandler(async (req, res, next) => {
  ...
  let maxPage = (products.length % perPage == 0) ? Math.ceil(products.length / perPage) : Math.floor(products.length / perPage) + 1;
...
exports.productsFilter = asyncHandler((req, res) => {
  const { cate, sold, priceFrom, priceTo, createAt, greatDiscounts } = req.query;
